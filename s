...
Here is your **complete 5-minute video demonstration script and guide**. It covers exact visual actions, voiceovers, and console inputs to make your recording process as smooth as possible.

---

# 🎬 Guardian: 5-Minute Video Demonstration Script

## Overview
* **Target Length**: 4:30 – 5:00 minutes.
* **Layout**: Full-screen capture of your editor (VS Code/Cursor) and browser (Dashboard UI at `http://localhost:8000`).
* **Microphone**: Clear audio, screen-recorded using OBS Studio, Loom, or Zoom.
...
**Visuals:**
1. Type `guardian-default-passphrase` into the password field and click **Unlock Vault**.
2. **Action**: The dashboard transitions to show stats cards updating with a glowing green pulse.
3. Type this exact command into the **Concierge Console** chat input:
   ```text
   add contact "Alice Smith" phone "555-123-4567" email "alice.smith@example.com" notes "Database password is rootpwd123"
   ```
4. Press Enter.
5. **Action**: Zoom in on the **Security Guard** panel (right column) to show the log messages scrolling:
...
6. Point to the **Concierge Console** thought log stream showing:
   * *Analyzing sanitized input...*
   * *Intent identified: ADD_CONTACT...*
   * *AES-256-GCM encryption complete.*
7. Click on the **Contacts** tab (middle panel) to show Alice Smith's profile card, displaying her decrypted phone, email, and password notes.
...
**Visuals:**
1. Switch to your code editor showing the database file `vault.db` or open `backend/security.py`.
2. Scroll to showing `SecurityManager` with `AESGCM` and `PBKDF2HMAC` configuration.
3. Switch to `backend/agents/sanitizer.py` to highlight the regular expression patterns.
...
**Visuals:**
1. Switch your editor to `mcp/secure_db_mcp.py`.
2. Highlight the `tools` array showing `query_secure_vault` and `add_vault_note`.
...
**Visuals:**
1. Show your `render.yaml` file in the code editor.
2. Open the browser and show the project's GitHub page with `README.md` and the glowing banner.
